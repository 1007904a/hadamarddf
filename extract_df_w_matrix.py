...
# get hadamard codes
_m = 2048  # output's vector from resnet101 ...  
codes = hadamard(_m)
mtrx_W = codes[:, :1024].T

idxs = np.where(mtrx_W <= 0)
...
def main(model_class, files_dir, ef):
    ...
    for _dd_idx, (imgs, labels) in enumerate(dataloader_dict):
        ...
        for i, output in enumerate(outputs):
            ...
            out_ = np.dot(mtrx_W, out_)
            out2_ = np.zeros(out_.size)
            _idx = np.where(out_ > 0)[0]
            out2_[_idx] = 1
...
if __name__ == "__main__":
    ...
    for idx in [5]: # use idxs from 'models_idx'
        base_model = "w_{}".format(models_name[idx])
