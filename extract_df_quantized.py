...
def quantize(value, bits=16):
    ...
    min_value = -2**(bits-1)
    step_size = (max_value - min_value) / (2**bits - 1)

    quantized_value = round(value / step_size) * step_size

    if quantized_value > max_value:
        quantized_value = max_value
    elif quantized_value < min_value:
...
def main(model_class, files_dir, ef):
    ...
    for _dd_idx, (imgs, labels) in enumerate(dataloader_dict):
        ...
        for i, output in enumerate(outputs):
            ...
            out_ = np.array(list(map(lambda x: quantize(x, bits=8), out_))) # 16, 8, 4, 2 bits 
